...
class ViewController: UIViewController {
    ...
    fileprivate func registerForFireBaseUpdates()
    {
        self.listener = self.ref?.addSnapshotListener({ (snapshot, error) in
            ...
            self.entries.append(LocationLookup(origLat: origLat!, origLng: origLng!, destLat: destLat!,
            destLng: destLng!, timestamp: (timestamp?.dateFromISO8601)!))
    ...
    func doCalculatations()
    {
        ...
        if let r = self.ref {
             ...
             r.addDocument(data: self.toDictionary(vals: entry))
         }
    }
    func toDictionary(vals: LocationLookup) -> [String:Any] {
            return [
